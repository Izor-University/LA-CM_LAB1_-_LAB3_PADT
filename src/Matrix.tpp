...
Matrix<T>::Matrix(int rows, int cols) : rows(rows), cols(cols) {
    if (rows <= 0 || cols <= 0) {
        throw IndexOutOfRange("Dimensions must be positive");
    }
...
double Matrix<T>::Norm() const {
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        for (int j = 0; j < cols; ++j) {
            currentSum += MathUtils::Abs((*this)(i, j));
        }
...
void Matrix<T>::AddRowMultiple(int sourceRow, int targetRow, const T& factor) {
    ...
    for (int j = 0; j < cols; ++j) {
        (*this)(targetRow, j) = (*this)(targetRow, j) + (*this)(sourceRow, j) * factor;
    }
}
